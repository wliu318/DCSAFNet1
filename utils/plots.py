...
def plot_images_labels_out(images, targets, pred_out, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16):
    # Plot image grid with labels
    ...
    for i, img in enumerate(images):
        ...
        if len(pred_out) > 0:
            image_pred_out = pred_out[pred_out[:, 0] == i]
            boxes = xywh2xyxy(image_pred_out[:, 2:6]).T
            classes = image_pred_out[:, 1].astype('int')
            labels = image_pred_out.shape[1] == 6  # labels if no conf column
            conf = None if labels else image_pred_out[:, 6]  # check for confidence presence (label vs pred)
            ...
            for j, box in enumerate(boxes.T):
                ...
                color = colors(2)
                cls = names[cls] if names else cls
                if conf[j] > 0.25 : #and int(classes[j]) < 3:  # labels or 0.25 conf thresh
                    # label = '%s' % cls if labels else '%s %.1f' % (cls, conf[j])
                    label = 'o'
                    plot_one_box(box, mosaic, label=None, color=color, line_thickness=tl)
                    # plot_one_box(box, mosaic, label=label, color=color, line_thickness=tl)   20240820,liuwen
