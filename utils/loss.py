...
class BCEBlurWithLogitsLoss(nn.Module):
    ...
    def __init__(self, alpha=0.05):
        ...
        #当你设置 reduction='none' 时，这个损失函数会为每一个输入样本独立地计算损失，而不是对所有的损失进行某种形式的聚合（如平均或求和）。
        self.loss_fcn = nn.BCEWithLogitsLoss(reduction='none')  # must be nn.BCEWithLogitsLoss()。结合了sigmoid激活函数和二元交叉熵损失（Binary Cross Entropy Loss）的类。这个损失函数常用于二分类问题，其中模型输出的是原始得分（logits）而不是概率。
        self.alpha = alpha
...
class QFocalLoss(nn.Module):
    ...
    def forward(self, pred, true):
        ...
        pred_prob = torch.sigmoid(pred)  # prob from logits
        alpha_factor = true * self.alpha + (1 - true) * (1 - self.alpha)
        modulating_factor = torch.abs(true - pred_prob) ** self.gamma  #** 是Python中的幂运算符，用于计算一个数的幂。
        loss *= alpha_factor * modulating_factor
        #当 self.gamma 大于1时，较大的差值会被放大，从而增加调制因子的值。这通常用于强调那些预测非常不准确的样本。
        #当 self.gamma 接近0时，差值对调制因子的影响会减小，接近于一个常数。这可能导致所有样本的调制因子几乎相同，从而失去了强调预测不准确样本的效果。
...
class VFLoss(nn.Module):
    ...
    def forward(self, pred, true):
        ...
        #true * (true > 0.0).float() 通常用于将布尔值（true）tensor转换为浮点数tensor，并且只保留那些原本为True的元素，而将False的元素转换为0。
